...
// Small read-only MCP server for tools that help inspect this local project.
#[derive(Clone)]
...
#[tool_router]
impl ProjectServer {
    // Describe static project facts without touching the filesystem.
    ...
    fn describe_project(&self) -> String {
        [
            ...
            "Hot-reloadable drawing code lives at rust/crates/hot_reload/lib.rs.",
            "The hot reload script is scripts/main/RunDesktopWithHotReload.ps1.",
        ]
        .join("\n")
    }
    ...
    fn hot_reload_target(&self) -> String {
        "rust/crates/hot_reload/lib.rs".to_string()
    }

    // Commands are returned as documentation only; this MCP tool does not run them.
    ...
    fn read_project_resource(&self, Parameters(request): Parameters<ResourceRequest>) -> String {
        // Keep resource access whitelisted so the server stays read-only and scoped.
    ...
    fn take_screenshot(&self, Parameters(request): Parameters<ScreenshotRequest>) -> String {
        // Resolve relative paths from the caller's current directory.
...
fn screenshot_script(output_path: &PathBuf) -> String {
    ...
    // PowerShell handles the Windows-only DPI and foreground-window APIs.
    format!(
        r#"
...
fn ps_single_quoted(value: &str) -> String {
    // Escape single quotes for safe insertion into a single-quoted PS string.
...
async fn main() -> Result<()> {
    // Serve over stdio so Codex or another MCP client can launch the process.
